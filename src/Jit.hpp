...
/*=== Relative jump offsets & jump labels ===*/
struct Rel{
    unsigned char* addr = nullptr;
    explicit Rel(unsigned char* addr) : addr(addr) {}
};
...
public:
    ...
    inline void PUSH (GPR64 r) { R(r); A(1,mp); O<64>(mp); W(0x50+r); R64FXDBG; }
    inline void POP  (GPR64 r) { R(r); A(1,mp); O<64>(mp); W(0x58+r); R64FXDBG; }

    inline void JMP  (GPR64 r) { B(r); A(2,mp); O<64>(mp); W(0xFF, ModRM(3, 4, r)); R64FXDBG; }
    inline void JMP  (Mem64 m) { A(6); W(0xFF, ModRM(0, 4, 5)); RIP(m.addr); R64FXDBG; }

    inline void JMP  (JumpLabel8  &j) { A(2); W(0xEB); LABEL(j); R64FXDBG; };
    inline void JMP  (JumpLabel32 &j) { A(5); W(0xE9); LABEL(j); R64FXDBG; };

    /* Generated by gen_jit.sh script */
#   include "JitInstructions_x86.cxx"

#   define DEF_CMPPS(C, N)\
    template<typename SrcT> inline void  CMP##C##PS (Xmm d, SrcT s)          { CMPPS  (d, s, Imm8(N));      R64FXDBG; }\
    template<typename SrcT> inline void VCMP##C##PS (Xmm d, Xmm s1, SrcT s2) { VCMPPS (d, s1, s2, Imm8(N)); R64FXDBG; }\
    template<typename SrcT> inline void VCMP##C##PS (Ymm d, Ymm s1, SrcT s2) { VCMPPS (d, s1, s2, Imm8(N)); R64FXDBG; }
...
#   define DEF_Jcc(CC, OPCODE)                      \
    inline void CC(Rel r)                           \
    {                                               \
        long offset = long(r.addr) - long(p) - 2;   \
                                                    \
