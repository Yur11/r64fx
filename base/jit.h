...
public:
    Assembler(int npages) : bytes(npages)
    {
...
/* ========== Generated from ./encoding.csv file ========================== */
    
    inline void add(GPR8 reg1, unsigned char byte2)
    {
        bytes << (0x80) << ModRM(b11, 0, reg1.code());
        bytes << Imm8(byte2);
    }

    inline void add(GPR16 reg1, unsigned short word2)
    {
        bytes << 0x66 << (0x81) << ModRM(b11, 0, reg1.code());
        bytes << Imm16(word2);
    }

    inline void add(GPR32 reg1, unsigned int dword2)
    {
        bytes << (0x81) << ModRM(b11, 0, reg1.code());
        bytes << Imm32(dword2);
    }

    inline void add(GPR64 reg1, unsigned int dword2)
    {
        bytes << Rex(1, 0, 0, reg1.prefix_bit()) << (0x81) << ModRM(b11, 0, reg1.code());
        bytes << Imm32(dword2);
    }

    inline void add(GPR8 reg1, GPR8 reg2)
    {
        bytes << (0x02) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void add(GPR16 reg1, GPR16 reg2)
    {
        bytes << 0x66 << (0x03) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void add(GPR32 reg1, GPR32 reg2)
    {
        bytes << (0x03) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void add(GPR64 reg1, GPR64 reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x03) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void add(GPR8 reg1, Mem8 mem2)
    {
        bytes << (0x02) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void add(GPR16 reg1, Mem16 mem2)
    {
        bytes << 0x66 << (0x03) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void add(GPR32 reg1, Mem32 mem2)
    {
        bytes << (0x03) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void add(GPR64 reg1, Mem64 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x03) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void add(Mem8 mem1, GPR8 reg2)
    {
        bytes << (0x00) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void add(Mem16 mem1, GPR16 reg2)
    {
        bytes << 0x66 << (0x01) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void add(Mem32 mem1, GPR32 reg2)
    {
        bytes << (0x01) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void add(Mem64 mem1, GPR64 reg2)
    {
        bytes << Rex(1, reg2.prefix_bit(), 0, 0) << (0x01) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void add(Mem8 mem1, unsigned char byte2)
    {
        bytes << (0x80) << ModRM(b00, 0, b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4 + sizeof(byte2));
        bytes << Imm8(byte2);
    }

    inline void add(Mem16 mem1, unsigned short word2)
    {
        bytes << 0x66 << (0x81) << ModRM(b00, 0, b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4 + sizeof(word2));
        bytes << Imm16(word2);
    }

    inline void add(Mem32 mem1, unsigned int dword2)
    {
        bytes << (0x81) << ModRM(b00, 0, b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4 + sizeof(dword2));
        bytes << Imm32(dword2);
    }

    inline void add(Mem64 mem1, unsigned int dword2)
    {
        bytes << Rex(1, 0, 0, 0) << (0x81) << ModRM(b00, 0, b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4 + sizeof(dword2));
        bytes << Imm32(dword2);
    }

    inline void mov(GPR8 reg1, unsigned char byte2)
    {
        bytes << (0xB0 + (reg1.code() & b0111)) << Imm8(byte2);
    }

    inline void mov(GPR16 reg1, unsigned short word2)
    {
        bytes << 0x66 << (0xB8 + (reg1.code() & b0111)) << Imm16(word2);
    }

    inline void mov(GPR32 reg1, unsigned int dword2)
    {
        bytes << (0xB8 + (reg1.code() & b0111)) << Imm32(dword2);
    }

    inline void mov(GPR64 reg1, unsigned long long qword2)
    {
        bytes << Rex(1, 0, 0, reg1.prefix_bit()) << (0xB8 + (reg1.code() & b0111)) << Imm64(qword2);
    }

    inline void mov(GPR8 reg1, GPR8 reg2)
    {
        bytes << (0x8A) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void mov(GPR16 reg1, GPR16 reg2)
    {
        bytes << 0x66 << (0x8B) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void mov(GPR32 reg1, GPR32 reg2)
    {
        bytes << (0x8B) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void mov(GPR64 reg1, GPR64 reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x8B) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void mov(GPR8 reg1, Mem8 mem2)
    {
        bytes << (0x8A) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void mov(GPR16 reg1, Mem16 mem2)
    {
        bytes << 0x66 << (0x8B) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void mov(GPR32 reg1, Mem32 mem2)
    {
        bytes << (0x8B) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void mov(GPR64 reg1, Mem64 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x8B) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void mov(Mem8 mem1, GPR8 reg2)
    {
        bytes << (0x88) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void mov(Mem16 mem1, GPR16 reg2)
    {
        bytes << 0x66 << (0x89) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void mov(Mem32 mem1, GPR32 reg2)
    {
        bytes << (0x89) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void mov(Mem64 mem1, GPR64 reg2)
    {
        bytes << Rex(1, reg2.prefix_bit(), 0, 0) << (0x89) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void mov(Mem8 mem1, unsigned char byte2)
    {
        bytes << (0xC6) << ModRM(b00, 0, b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4 + sizeof(byte2));
        bytes << Imm8(byte2);
    }

    inline void mov(Mem16 mem1, unsigned short word2)
    {
        bytes << 0x66 << (0xC7) << ModRM(b00, 0, b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4 + sizeof(word2));
        bytes << Imm16(word2);
    }

    inline void mov(Mem32 mem1, unsigned int dword2)
    {
        bytes << (0xC7) << ModRM(b00, 0, b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4 + sizeof(dword2));
        bytes << Imm32(dword2);
    }

    inline void mov(Mem64 mem1, unsigned int dword2)
    {
        bytes << Rex(1, 0, 0, 0) << (0xC7) << ModRM(b00, 0, b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4 + sizeof(dword2));
        bytes << Imm32(dword2);
    }

    inline void sub(GPR8 reg1, unsigned char byte2)
    {
        bytes << (0x80) << ModRM(b11, 5, reg1.code());
        bytes << Imm8(byte2);
    }

    inline void sub(GPR16 reg1, unsigned short word2)
    {
        bytes << 0x66 << (0x81) << ModRM(b11, 5, reg1.code());
        bytes << Imm16(word2);
    }

    inline void sub(GPR32 reg1, unsigned int dword2)
    {
        bytes << (0x81) << ModRM(b11, 5, reg1.code());
        bytes << Imm32(dword2);
    }

    inline void sub(GPR64 reg1, unsigned int dword2)
    {
        bytes << Rex(1, 0, 0, reg1.prefix_bit()) << (0x81) << ModRM(b11, 5, reg1.code());
        bytes << Imm32(dword2);
    }

    inline void sub(GPR8 reg1, GPR8 reg2)
    {
        bytes << (0x2A) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void sub(GPR16 reg1, GPR16 reg2)
    {
        bytes << 0x66 << (0x2B) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void sub(GPR32 reg1, GPR32 reg2)
    {
        bytes << (0x2B) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void sub(GPR64 reg1, GPR64 reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x2B) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void sub(GPR8 reg1, Mem8 mem2)
    {
        bytes << (0x2A) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void sub(GPR16 reg1, Mem16 mem2)
    {
        bytes << 0x66 << (0x2B) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void sub(GPR32 reg1, Mem32 mem2)
    {
        bytes << (0x2B) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void sub(GPR64 reg1, Mem64 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x2B) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void sub(Mem8 mem1, GPR8 reg2)
    {
        bytes << (0x28) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void sub(Mem16 mem1, GPR16 reg2)
    {
        bytes << 0x66 << (0x29) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void sub(Mem32 mem1, GPR32 reg2)
    {
        bytes << (0x29) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void sub(Mem64 mem1, GPR64 reg2)
    {
        bytes << Rex(1, reg2.prefix_bit(), 0, 0) << (0x29) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void sub(Mem8 mem1, unsigned char byte2)
    {
        bytes << (0x80) << ModRM(b00, 5, b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4 + sizeof(byte2));
        bytes << Imm8(byte2);
    }

    inline void sub(Mem16 mem1, unsigned short word2)
    {
        bytes << 0x66 << (0x81) << ModRM(b00, 5, b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4 + sizeof(word2));
        bytes << Imm16(word2);
    }

    inline void sub(Mem32 mem1, unsigned int dword2)
    {
        bytes << (0x81) << ModRM(b00, 5, b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4 + sizeof(dword2));
        bytes << Imm32(dword2);
    }

    inline void sub(Mem64 mem1, unsigned int dword2)
    {
        bytes << Rex(1, 0, 0, 0) << (0x81) << ModRM(b00, 5, b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4 + sizeof(dword2));
        bytes << Imm32(dword2);
    }

    inline void addps(Xmm reg1, Mem128 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x0F) << (0x58) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void addps(Xmm reg1, Xmm reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0x58) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void subps(Xmm reg1, Mem128 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x0F) << (0x5C) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void subps(Xmm reg1, Xmm reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0x5C) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void mulps(Xmm reg1, Mem128 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x0F) << (0x59) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void mulps(Xmm reg1, Xmm reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0x59) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void divps(Xmm reg1, Mem128 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x0F) << (0x5E) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void divps(Xmm reg1, Xmm reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0x5E) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void rcpps(Xmm reg1, Mem128 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x0F) << (0x53) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void rcpps(Xmm reg1, Xmm reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0x53) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void sqrtps(Xmm reg1, Mem128 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x0F) << (0x51) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void sqrtps(Xmm reg1, Xmm reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0x51) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void rsqrtps(Xmm reg1, Mem128 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x0F) << (0x52) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void rsqrtps(Xmm reg1, Xmm reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0x52) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void maxps(Xmm reg1, Mem128 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x0F) << (0x5F) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void maxps(Xmm reg1, Xmm reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0x5F) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void minps(Xmm reg1, Mem128 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x0F) << (0x5D) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void minps(Xmm reg1, Xmm reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0x5D) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void andnps(Xmm reg1, Mem128 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x0F) << (0x55) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void andnps(Xmm reg1, Xmm reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0x55) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void andps(Xmm reg1, Mem128 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x0F) << (0x54) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void andps(Xmm reg1, Xmm reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0x54) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void cmpps(Xmm reg1, Xmm reg2, unsigned char byte)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0xC2) << ModRM(b11, reg1.code(), reg2.code());
        bytes << Imm8(byte);
    }

    inline void movaps(Xmm reg1, Mem128 mem2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, 0) << (0x0F) << (0x28) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void movaps(Xmm reg1, Xmm reg2)
    {
        bytes << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0x28) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void movaps(Mem128 mem1, Xmm reg2)
    {
        bytes << Rex(1, reg2.prefix_bit(), 0, 0) << (0x0F) << (0x29) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void movupd(Xmm reg1, Mem128 mem2)
    {
        bytes << 0x66 << Rex(1, reg1.prefix_bit(), 0, 0) << (0x0F) << (0x10) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void movupd(Xmm reg1, Xmm reg2)
    {
        bytes << 0x66 << Rex(1, reg1.prefix_bit(), 0, reg2.prefix_bit()) << (0x0F) << (0x10) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void movupd(Mem128 mem1, Xmm reg2)
    {
        bytes << 0x66 << Rex(1, reg2.prefix_bit(), 0, 0) << (0x0F) << (0x11) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }

    inline void vmovupd(Ymm reg1, Mem256 mem2)
    {
        bytes << Vex3(reg1.prefix_bit(), 0, 0, b00001, 0, 0, 1, b01) << (0x10) << ModRM(b00, reg1.code(), b101);
        bytes << Rip(mem2.addr, bytes.codeEnd() + 4);
    }

    inline void vmovupd(Ymm reg1, Ymm reg2)
    {
        bytes << Vex3(reg1.prefix_bit(), 0, reg2.prefix_bit(), b00001, 0, 0, 1, b01) << (0x10) << ModRM(b11, reg1.code(), reg2.code());
    }

    inline void vmovupd(Mem256 mem1, Ymm reg2)
    {
        bytes << Vex3(reg2.prefix_bit(), 0, 0, b00001, 0, 0, 1, b01) << (0x11) << ModRM(b00, reg2.code(), b101);
        bytes << Rip(mem1.addr, bytes.codeEnd() + 4);
    }
